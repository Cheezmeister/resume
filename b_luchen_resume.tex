...
\documentclass[centered,margin]{res}
...
\newcommand{\link}[2]{
  \href{#1}{\texttt \textul{#2}}
}

\newcommand{\footer}{
  % \hfill \textit{\small References are available upon request.}
  \vfill
  Handcrafted with \heartsuit + \link{https://github.com/Cheezmeister/resume/raw/master/b_luchen_resume.tex}{\LaTeX}
  \hfill \textit{This document lives at \link{http://resume.bml.rocks}{resume.bml.rocks} | created \timestamp }
}

\newcommand{\role}[5]{
  \begin{tabularx}
    \textbf{#1} (\link{https://#5}{#5}) & \hfill \textbf{#2} \\
    \textit{#3}   & \hfill #4          \\
  \end{tabularx}
}
...
% \name{\includegraphics[width=2cm, height=2cm]{logo.png} \huge Brendan Luchen}
\address{
  \texttt{brendan@\href{http://is.gd/bmlBio}{luchenlabs.com}} 
  \textperiodcentered \href{http://bml.rocks}{www.bml.rocks}
  |
  \href{https://linkedin.com/in/Cheezmeister}{linkedin.com/in/Cheezmeister}
  \textperiodcentered \href{http://is.gd/bmlGithub}{github.com/Cheezmeister}
}
...
% I try to avoid \href{https://www.fearlessculture.design/blog-posts/stop-solving-the-wrong-problem}{solving the wrong problem}.
% I have lived in \href{http://bml.rocks/#where}{a half-dozen US states}, three countries; and visited many more.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\sc Employer}
  \role
    {John Turner Consulting} {Portsmouth, NH}
    {Asphalt Technician} {August 2024--present}
    {consultjtc.com}
  \employerSummary {Strategic Engineering Solutions that Exceed Expectations}

\section{\sc Prior Employers}
  ...
  \role
    ...
    {archipelago-ai.com}
  \employerSummary
    {Work smarter, chat less.}
  \impact{
    \item Architected and implemented scalable microservices using Kotlin and Ktor
    \item Designed resilient database integration patterns using jOOQ and TiDB
    \item Integrated klogging to emit structured logs from the api server backend
    \item Developed and verified the first end-to-end integration between frontend, backend, and database layer
    \item Mentored grad student intern towards introducing Kotlin unit testing
    \item Collaborated with UX designer to establish processs and plan a Storybook-based design system
  }
  \technologies{
    Kotlin (Ktor, Klogging, Koin, jOOQ)
    TypeScript (NodeJS, SvelteKit, TailwindCSS)
    IaC (GitHub Actions, Ansible, Docker, Dagger CI)
    MySQL (TiDB)
  }
  ...
  \role
    ...
    {bcnc.net}
  \employerSummary {Adult Education and Workforce Initiatives.}
  \impact{
    \item Managed electronics inventory, offered live tech support, and streamlined IT operations
    \item Supported the resolution of a ~\$13,000 disputed bill with T-Mobile
  }
  \technologies{
    Squarespace, Zapier, Sharepoint, QR Codes
  }
  ...
  \role
    ...
    {teikametrics.com}
  \employerSummary
    {The AI-powered platform to optimize and grow your ecommerce business on Walmart \& Amazon.}
  \impact{
    \item Introduced i18n to the React frontend
    ...
    \item Designed and implemented a scalable and secure cloud-based microservice-logging system
    \item{Championed a dynamic, collaborative, safe, and healthy culture
    }
  }
  \technologies{
...
\section{\sc Past Employers}
    ...
    {
      Comcast Technology Solutions 
      (formerly ``\href{https://en.wikipedia.org/wiki/ThePlatform}{\textit{thePlatform}}'')
      delivers a media management and publishing platform to distribute video and other digital content.
    }
    \impact{
        \item Verified code end-to-end allowing Comcast subscribers on Apple TV to login with their Apple ID
    }
    ...
    \role
      ...
      {amazon.com}
    \employerSummary
      {The Platform Excellence in Fatals team is tasked 
       with bringing customer-facing failures ever closer to zero.}
    \impact{
        \item Drove company-wide initiatives to reduce HTTP 5xx and other failures
        \item Shared a 24/7 pager-duty rotation one out of every 3 weeks
    }
    ...
    \role {Activision} {Portland, Maine}
    {Programmer Intern} {June 2011--September 2011}
    {activision.com}

    \role {IBM} {Durham, North Carolina}
    {Software Engineering Co-op} {March 2009-September 2009}
    {ibm.com}
...
\section{\sc Shipped Software}
    \textbf{\href{http://ocjb.me}{\underline{OverClocked Jukebox}}}: Web jukebox for VGM arrangements from \texttt{ocremix.org} [ Elixir/Phoenix/JS ] \\
    \textbf{\href{https://cheezmeister.itch.io/vec}{\underline{Vec (2015)}}}: Procedural arcade game Windows/Mac/Linux [ C++/SDL/OpenGL ] \\
    \textbf{\href{https://is.gd/chromaReview}{\underline{Chromathud (2013)}}}: Educational puzzle game for PC and Xbox Live Indie Games [ \CS/.NET/XNA ] \\
    ...
    \textbf{Modern Warfare 3 (2011)}: Multiplatform console/PC FPS game (\copyright\ Activision) [ C++/idtech3 ] \\
    \textbf{Haiku for Android}: Thin client to Epic's EMR (\copyright\ Epic Systems Corporation) [ Java/Android/\CS ] \\
    \textbf{Flywheel}: The eCommerce Seller's Operating System (\copyright\ Teikametrics) [ TypeScript/Python/Postgres] \\
...
\section{\sc Skills}
    \begin{tabular}{@{\extracolsep{\fill} }r|lll}
      ...
      \hline
      \textbf{Programming}  & Java, Kotlin, Scala,     & Python, Ruby        & Perl, Haskell \\
                         & JS/TS/ES2015+            & C, \CS, C++         & Elixir, Elm, Golang \\
